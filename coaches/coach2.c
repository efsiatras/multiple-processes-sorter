...
int main(int argc, char *argv[]) {
	...
	if (argc < 5) {
		printf("Too few arguments.\nUsage: './coach2 inputfile numOfRecords typeOfSort columnid'\n");
	...
	long recordsLeft = numOfRecords / 8; // Number of records left to be transferred by each sorter
	...
	recordsLeft = numOfRecords / 4 - numOfRecords / 8; // Reset counter for next sorter
	...
	recordsLeft = numOfRecords / 2 - numOfRecords / 4; // Reset counter for next sorter
	while (recordsLeft > 0) { // While there are still records to be transferred from sorter1
		if (recordsLeft >= recordPipeCap) { // If there are records left more than (or equal) to the record pipe capacity
	...
	tsorteravg = (tsorter0 + tsorter1 + tsorter2 + tsorter3) / 4; // Calculate average time of sorters
